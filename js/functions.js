var functions = {
	...
	update() {
		...
		let Gv = data.totalA.log(Number.MAX_VALUE).root(4).log(2).mul(5).pow(1.5).add(1)
		...
		data.number = data.totalA.gte(Number.MAX_VALUE)
			...
			: data.totalA;
		data.layer = data.totalA.gte(Number.MAX_VALUE) ? hy.floor().add(1) : data.totalA.log(Number.MAX_VALUE).floor().add(1);
		...
		document.getElementsByClassName("layer-background")[0].style.background = data.layer.gte("10^^10") ? `linear-gradient(to bottom, hsl(${data.layer.slog().log10().mul(180)} 100 ${(50+(Math.sin(data.layer.slog().log10())*15)+15)/4}), #000)` : 
				...
				`linear-gradient(to bottom, hsl(${data.layer.log10().mul(180)} ${data.layer.gte("10") ? 100 : data.layer.mul(10)} 18), #000)`;
