var functions = {
	...
	update() {
		...
		data.number = data.totalA.gte(Number.MAX_VALUE)
			...
			: data.totalA;
		data.layer = data.totalA.gte(Number.MAX_VALUE) ? data.totalA.log(Number.MAX_VALUE).log10().add(1.1**25).log(1.1).tetrate(0.9).floor().sub(19) : data.totalA.log(Number.MAX_VALUE).floor().add(1);
