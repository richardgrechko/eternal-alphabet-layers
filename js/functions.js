var functions = {
	...
	update() {
		...
		document.getElementById("layers").innerHTML = data.layerHTML = data.layer.gte("10^^10") ? `<span style="color: hsl(${data.layer.slog().log10().mul(360)} 100 50); text-shadow: currentcolor 0 0 0.5em;">${this.convertToLayer(data.layer)}</span>` : 
				...
				`${data.number.toPrecision(4)}<span style="color: hsl(${data.layer.log10().mul(180)} ${data.layer.gte("10") ? 100 : data.layer.mul(10)} 50); text-shadow: currentcolor 0 0 ${data.layer.gte("100") ? 0.5 : data.layer.sqrt().div(20)}em;">${this.convertToLayer(data.layer)}</span>`;
